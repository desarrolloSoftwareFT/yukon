...
! ITL Test Source File:      digital/u8_connect_h
! Test Name:                 U8
! VCL created for chain:     U1_C_U1
! Date: Thu Jul 23 13:31:20 2020
...
assign N0000 to nodes "BMC_FAN_CARD_RST_R" ! Pin(s) V6
assign N0001 to nodes "BMC_GPIOS6_STRAP" ! Pin(s) U20
assign N0002 to nodes "BMC_RXD1_R" ! Pin(s) W1
assign N0003 to nodes "BMC_SPARE_IO2" ! Pin(s) U19
assign N0004 to nodes "BMC_SRST" ! Pin(s) U18
assign N0005 to nodes "BMC_TXD1_R" ! Pin(s) V1
assign N0006 to nodes "GPIOZ4_VPOG6_NORA4" ! Pin(s) U21
assign N0007 to nodes "GPIOZ6_VPOG8_NORA6" ! Pin(s) V22
assign N0008 to nodes "I2C_BUS11_ALERT_R" ! Pin(s) U22
...
assign N0009 to nodes "MEM_DDR4_ALERT_N" ! Pin(s) U16
assign N0010 to nodes "MEM_DDR4_MA<0>" ! Pin(s) V13
assign N0011 to nodes "MEM_DDR4_MA<10>" ! Pin(s) U13
assign N0013 to nodes "SPI2_CS1_R" ! Pin(s) V20
assign N0014 to nodes "SRT_BMC_SPI2_MUX_MISO" ! Pin(s) V19
assign N0012 to nodes "SRT_SPI_BOOT_MOSI" ! Pin(s) U17
...
inputs          N0004
outputs         N0010, N0011
bidirectional   N0000, N0001, N0002, N0003, N0005, N0006, N0007, N0008
bidirectional   N0009, N0013, N0014, N0012
...
use cards hybrid         on N0008, N0009, N0010, N0011, N0013, N0014, N0012
...
!LLLLLBBBBBBGGIMMMSSS!
!JJJJJMMMMMMPP2EEERPR!
!TTTTTCCCCCCIICMMMTIT!
!AAAAA______OO_____2_!
!GGGGGFGRSSTZZBDDDS_B!
!_____APXPRX46UDDDPCM!
!TTTTTNIDASD__SRRRISC!
!CMDDR_O1RT1VV1444_1_!
!KSIOSCS_E _PP1___B_S!
!____TA6R_ ROO_AMMORP!
!HHHH_R_ I  GGALAAO I!
!DDDDLDS O  68LE<<T 2!
!RRRR__T 2  __ER01_ _!
!____HRR    NNRT>0M M!
...
"01ZX0XXXXZXXXXXXXXXX"!Reset via TRST* and synchonizing sequence
use pcf order Scan
"11ZX0"
"01ZX1"
"11ZX1"
...
use pcf order Parallel
"00ZX1XXXXZXXXXXXXXXX"
"10ZX1XXXXZXXXXXXXXXX"!Run-Test/Idle
"01ZX1XXXXZXXXXXXXXXX"
use pcf order Scan
"11ZX1"!Select-DR-Scan
use pcf order Parallel
...
use pcf order Parallel
"00ZX1XXXXZXXXXXXXXXX"
"10ZX1XXXXZXXXXXXXXXX"!Shift-DR Vector 125
end pcf
message "IEEE Std 1149.1-2001 Integrity Failure"
message "  Device U1 has failed,"
message "  suspect device or these pins:"
...
use pcf order Parallel
"00ZX1XXXXZXXXXXXXXXX"
"10ZX1XXXXZXXXXXXXXXX"!Run-Test/Idle Vector 450
"01ZX1XXXXZXXXXXXXXXX"
use pcf order Scan
"11ZX1"!Select-DR-Scan
"01ZX1"
...
"10ZX1"!Capture-DR
use pcf order Parallel
"00ZX10000000000XX000"
"10ZX10000000000XX000"!Shift-DR
"10ZX1Z000000000XX000"!Disable drivers sequentially. 
"10ZX1ZZ00000000XX000"
"10ZX1ZZZ0000000XX000"
"10ZX1ZZZZ000000XX000"
"10ZX1ZZZZZ00000XX000"
"10ZX1ZZZZZZ0000XX000"
"10ZX1ZZZZZZZ000XX000"
"10ZX1ZZZZZZZZ00XX000"
"10ZX1ZZZZZZZZZ0XX000"
"10ZX1ZZZZZZZZZZXX000"
"10ZX1ZZZZZZZZZZXXZ00"
"10ZX1ZZZZZZZZZZXXZZ0"
"10ZX1ZZZZZZZZZZXXZZZ"
...
"10ZX1"!Suppress Ground Bounce
use pcf order Parallel
"00ZX1XXXXZXXXXXXXXXX"! Vector 3850
"10ZX1XXXXZXXXXXXXXXX"!Run-Test/Idle
"01ZX1XXXXZXXXXXXXXXX"
use pcf order Scan
"11ZX1"!Select-DR-Scan
...
"10ZX1"!Capture-DR
use pcf order Parallel
"00ZX11111111111XX111"
"10ZX11111111111XX111"!Shift-DR
"10ZX1Z111111111XX111"!Disable drivers sequentially. 
"10ZX1ZZ11111111XX111"
"10ZX1ZZZ1111111XX111"
"10ZX1ZZZZ111111XX111"
"10ZX1ZZZZZ11111XX111"
"10ZX1ZZZZZZ1111XX111"
"10ZX1ZZZZZZZ111XX111"
"10ZX1ZZZZZZZZ11XX111"
"10ZX1ZZZZZZZZZ1XX111"
"10ZX1ZZZZZZZZZZXX111"
"10ZX1ZZZZZZZZZZXXZ11"
"10ZX1ZZZZZZZZZZXXZZ1"
"10ZX1ZZZZZZZZZZXXZZZ"
! Load 4 Sentinel bits into chain registers.
! This shifts out the first 4 bits of capture data for U1.
end pcf
...
use pcf order Parallel
"00ZX1XXXXZXXXXXXXXXX"
"10ZX1XXXXZXXXXXXXXXX"!Run-Test/Idle Vector 5475
"01ZX1XXXXZXXXXXXXXXX"
use pcf order Scan
"11ZX1"!Select-DR-Scan
...
"10ZX1"!Capture-DR
use pcf order Parallel
"00ZX1ZZZZZZZZZZXXZZZ"
"10ZX1ZZZZZZZZZZXXZZZ"!Shift-DR
! Load 4 Sentinel bits into chain registers.
! This shifts out the first 4 bits of capture data for U1.
end pcf
compress
...
"10ZX1"!Capture-DR
use pcf order Parallel
"00ZX1ZZZZZZZZZZLLZZZ"
"10ZX1ZZZZZZZZZZLLZZZ"!Shift-DR
! Load 4 Sentinel bits into chain registers.
! This shifts out the first 4 bits of capture data for U1.
end pcf
compress
...
end unit ! Connect Test Vector 8705
! Vectors with TDI High:    3146,   3.1 milliseconds
! Vectors with TDI Low:      168, 168.0 microseconds
! Total Vectors :           8705,   8.7 milliseconds

! Connect Test Dictionary
! Frame Size 796
! FrCell DevCell Dev.Pin Node             Signature
!  330 328       U8.U22  I2C_BUS11_ALERT_R'LHXX'
!  339 337       U8.V22  GPIOZ6_VPOG8_NORA6'LHXX'
!  345 343       U8.U21  GPIOZ4_VPOG6_NORA4'LHXX'
!  372 370       U8.U20  BMC_GPIOS6_STRAP'LHXX'
!  378 376       U8.U19  BMC_SPARE_IO2   'LHXX'
!  381 379       U8.V20  SPI2_CS1_R      'LHXX'
!  384 382       U8.V19  SRT_BMC_SPI2_MUX_MISO'LHXX'
!  403 401       U8.U18  BMC_SRST        'LHXX'
!  475 473       U8.U17  SRT_SPI_BOOT_MOSI'LHXX'
!  480 478       U8.U16  MEM_DDR4_ALERT_N'LHXX'
!  595 593       U8.V6   BMC_FAN_CARD_RST_R'LHXX'
!  673 671       U8.W1   BMC_RXD1_R      'LHXX'
!  676 674       U8.V1   BMC_TXD1_R      'LHXX'
